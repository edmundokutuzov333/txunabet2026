...
const allowedModules = new Set<ModuleName>([
  'cloud', 'calendar', 'meetings', 'integrations', 'knowledge-base', 'campaigns', 'tasks', 'reports',
  'workflows', 'automations', 'pulse', 'workspaces',
]);
const CONTROL_PLANE_MODULES = new Set<ModuleName>(['workflows', 'automations']);
...
function assertGenericMutationAllowed(module: ModuleName): void {
  if (CONTROL_PLANE_MODULES.has(module)) throw new Error('CONTROL_PLANE_REQUIRED');
}

function errorResponse(error: unknown) {
  if (isAuthorizationError(error)) return NextResponse.json({ error: 'Acesso não autorizado.' }, { status: 403 });
  const code = error instanceof Error ? error.message : 'INTERNAL_ERROR';
  const status = ['MODULE_NOT_FOUND', 'INVALID_ID', 'INVALID_PAYLOAD', 'CONTROL_PLANE_REQUIRED'].includes(code) ? 400 : code === 'NOT_FOUND' ? 404 : 500;
...
export async function POST(request: NextRequest, context: { params: Promise<{ module: string }> }) {
  try {
    const { module: moduleParam } = await context.params;
    const module = parseModule(moduleParam);
    assertGenericMutationAllowed(module);
