...
import { assertFileSize } from '@/server/services/storage';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    storagePath = `companies/${identity.companyId}/users/${identity.uid}/files/${ref.id}-${safeName(file.name)}`;
    ...
    if (storagePath) await getAdminStorage().bucket().file(storagePath).delete({ ignoreNotFound: true }).catch(() => undefined);
