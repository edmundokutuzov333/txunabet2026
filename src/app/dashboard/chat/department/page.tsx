...
export default function DepartmentChatPage() {
  ...
  useEffect(() => {
    const departmentId = identity?.departmentIds?.[0];
    if (!departmentId) return;
    ensureConversation({ type: 'department', departmentId, name: 'Departamento' })
      .then((result) => setConversation(result.data))
      .catch((cause) => setError(cause instanceof Error ? cause.message : 'Não foi possível abrir o canal do departamento.'));
  }, [identity]);
  ...
  return <EnterpriseChatView conversation={conversation} title="Chat do Departamento" subtitle="Canal privado para os membros do seu departamento." />;
}
