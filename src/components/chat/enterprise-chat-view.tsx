...
import { Loader2, Paperclip, Pin, Search, Send, Trash2, Pencil, Wifi, WifiOff, Sparkles } from 'lucide-react';
...
export default function EnterpriseChatView({ conversation, title, subtitle }: Props) {
  ...
  const runSummary = async () => { setSummarizing(true); try { const response = await fetch(`/api/chat/conversations/${conversation.id}/summary`, { method: 'POST', credentials: 'include' }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error); setSummary(payload.summary); } catch (error) { toast({ title: 'Resumo indisponível', description: error instanceof Error ? error.message : 'Tente novamente.', variant: 'destructive' }); } finally { setSummarizing(false); } };

  return <div className="flex h-full min-h-0 flex-col p-6 gap-4">
    <div className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-bold">{title}</h1><p className="text-sm text-muted-foreground">{subtitle ?? 'Comunicação empresarial em tempo real.'}</p></div><div className="flex items-center gap-3"><Button variant="outline" onClick={runSummary} disabled={summarizing}>{summarizing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />} Resumir</Button><div className="flex items-center gap-2 text-xs text-muted-foreground">{online ? <Wifi className="h-4 w-4" /> : <WifiOff className="h-4 w-4 text-destructive" />}{online ? 'Ligado' : 'Sem ligação'}</div></div></div>
    ...
    {summary && <div className="rounded-xl border bg-primary/5 p-4 text-sm"><div className="font-semibold mb-1">Resumo Gemini</div><p className="whitespace-pre-wrap text-muted-foreground">{summary}</p></div>}
