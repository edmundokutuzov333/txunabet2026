...
import { AlertTriangle, ArrowRight, Bot, CheckCircle2, Clock3, Inbox, ListTodo, MessageSquare, ShieldAlert, Sparkles, Target, Users, Video, XCircle } from 'lucide-react';
...
export default function CommandCenterView() {
  ...
  return <div className="p-6 fade-in space-y-6">
    ...
    <section><h2 className="text-lg font-semibold mb-3">Attention</h2><div className="grid grid-cols-1 lg:grid-cols-5 gap-3">{[
      ['Critical', data.attention.critical, ShieldAlert], ['Needs action', data.attention.needsAction, ArrowRight], ['At risk', data.attention.atRisk, AlertTriangle], ['Blocked', data.attention.blocked, XCircle], ['Waiting', data.attention.waiting, Clock3],
    ].map(([title, items, Icon]) => <Card key={String(title)}><CardHeader className="pb-3"><CardTitle className="text-sm flex justify-between">{String(title)}<Badge variant="secondary">{(items as any[]).length}</Badge></CardTitle></CardHeader><CardContent className="space-y-2">{(items as any[]).slice(0,3).map((item) => <Item key={String(item.id)} item={item} icon={Icon as any} />)}{!(items as any[]).length && <p className="text-xs text-muted-foreground">Nada a destacar.</p>}</CardContent></Card>)}
    </div></section>

    <section className="grid grid-cols-1 xl:grid-cols-2 gap-5"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-primary" />AI Briefing</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">{data.briefing.summary}</p><div className="grid grid-cols-2 gap-3 text-sm"><div className="rounded-xl bg-muted/30 p-3"><p className="text-muted-foreground">Mudou</p><strong>{data.briefing.changed.length}</strong></div><div className="rounded-xl bg-muted/30 p-3"><p className="text-muted-foreground">Correu mal</p><strong>{data.briefing.failed.length}</strong></div><div className="rounded-xl bg-muted/30 p-3"><p className="text-muted-foreground">Concluído</p><strong>{data.briefing.completed.length}</strong></div><div className="rounded-xl bg-muted/30 p-3"><p className="text-muted-foreground">Em risco</p><strong>{data.briefing.atRisk.length}</strong></div></div><Separator /><div className="space-y-2">{data.briefing.changed.slice(0,4).map((item) => <Item key={String(item.id)} item={item} icon={Bot} />)}</div></CardContent></Card><Card><CardHeader><CardTitle className="flex items-center gap-2"><ActivityIcon /><span>Activity Feed</span></CardTitle></CardHeader><CardContent className="space-y-2">{data.activityFeed.slice(0,8).map((item) => <Item key={String(item.id)} item={item} icon={CheckCircle2} />)}{!data.activityFeed.length && <p className="text-sm text-muted-foreground">Ainda não há atividade normalizada.</p>}</CardContent></Card></section>
  </div>;
}
