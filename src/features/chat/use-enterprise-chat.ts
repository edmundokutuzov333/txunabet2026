...
function stableConversationId(type: EnterpriseConversation['type'], companyId: string, departmentId?: string, targetUserId?: string) {
  if (type === 'company_general') return `company_general_${companyId}`;
  if (type === 'department' && departmentId) return `department_${departmentId}`;
  if (type === 'direct' && targetUserId) return `direct_${[companyId, targetUserId].sort().join('_')}`;
  return null;
}
...
export function useEnterpriseChat(conversationId: string | null) {
  ...
  useEffect(() => {
    if (!conversationId || !user || !identity) {
      setMessages([]);
      setLoading(false);
      return;
    }
  ...
  const loadOlder = useCallback(async () => {
    ...
    try {
      const { firestore } = initializeFirebase();
      const messagesRef = collection(firestore, 'conversations', conversationId, 'messages');
      const olderQuery = query(messagesRef, orderBy('createdAt', 'asc'), startAfter(oldestSnapshot.current), limitToLast(50));
  ...
  const sendMessage = useCallback(async (input: {
    ...
    if (!response.ok) {
      ...
      const error = await response.json().catch(() => ({ error: 'Não foi possível enviar a mensagem.' }));
      throw new Error(error.error || 'Não foi possível enviar a mensagem.');
    }
    pending.current.delete(clientMessageId);
  }, [conversationId, identity, user]);
  ...
  const markRead = useCallback(async (messageId: string) => {
    ...
    await fetch(`/api/chat/conversations/${conversationId}/read`, {
      method: 'POST', headers: { 'content-type': 'application/json' }, credentials: 'include', body: JSON.stringify({ messageId }),
    });
  }, [conversationId]);
  ...
  const editMessage = useCallback(async (messageId: string, body: string) => {
    ...
    if (!response.ok) throw new Error('Não foi possível editar a mensagem.');
  }, [conversationId]);

  const deleteMessage = useCallback(async (messageId: string) => {
    if (!conversationId) return;
    const response = await fetch(`/api/chat/conversations/${conversationId}/messages/${messageId}`, { method: 'DELETE', credentials: 'include' });
    if (!response.ok) throw new Error('Não foi possível apagar a mensagem.');
  }, [conversationId]);

  const reactToMessage = useCallback(async (messageId: string, emoji: string, action: 'add' | 'remove') => {
    if (!conversationId) return;
    const response = await fetch(`/api/chat/conversations/${conversationId}/reactions`, {
      method: 'POST', headers: { 'content-type': 'application/json' }, credentials: 'include', body: JSON.stringify({ messageId, emoji, action }),
    });
    if (!response.ok) throw new Error('Não foi possível actualizar a reacção.');
  }, [conversationId]);

  return useMemo(() => ({ messages, loading, loadingOlder, hasMore, loadOlder, sendMessage, markRead, editMessage, deleteMessage, reactToMessage }), [messages, loading, loadingOlder, hasMore, loadOlder, sendMessage, markRead, editMessage, deleteMessage, reactToMessage]);
}
...
export function conversationIdFor(input: { type: EnterpriseConversation['type']; companyId: string; departmentId?: string; targetUserId?: string }) {
  return stableConversationId(input.type, input.companyId, input.departmentId, input.targetUserId);
}
