...
async function resolveContext(companyId: string, type: AIContextType, id?: string): Promise<string> {
  ...
  const collection = type === 'campaign' ? 'module_campaigns' : 'module_tasks';
...
async function generateWithControls(params: { uid: string; companyId: string; action: AIAction; prompt: string; context: string }): Promise<string> {
  ...
  const result = await runAIGateway({ agent: 'OryonAI', identity: { uid: params.uid, companyId: params.companyId } as never, tier: 'balanced', instruction: `${prompt}\n\nOperação: ${params.action}`, context });
  recordMetric({ name: 'ai_request', value: result.usageEstimate, success: true, userId: params.uid, companyId: params.companyId });
...
export async function runContextualAI(input: { uid: string; companyId: string; action: AIAction; prompt: string; contextType?: AIContextType; contextId?: string }) {
  ...
  const context = await resolveContext(input.companyId, contextType, input.contextId);
  const suggestion = await generateWithControls({ uid: input.uid, companyId: input.companyId, action: input.action, prompt: input.prompt, context });
...
export async function summarizeConversationSecure(uid: string, companyId: string, conversationId: string, action: 'summarize' | 'decisions' | 'tasks' | 'briefing' | 'pending') {
  ...
  const context = await resolveContext(companyId, 'conversation', conversationId);
  ...
  return generateWithControls({ uid, companyId, action: mapped, prompt: `${request} Não invente informação ausente.`, context });
}
