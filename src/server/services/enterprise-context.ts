...
const SOURCES: Source[] = [
  { type: 'people', collection: 'members', permission: PERMISSIONS.IDENTITY_READ, titleFields: ['displayName', 'name', 'email'], textFields: ['displayName', 'name', 'email', 'department', 'role'], special: 'people' },
  ...
  { type: 'documents', collection: 'module_documents', permission: PERMISSIONS.KNOWLEDGE_READ, titleFields: ['title', 'name'], textFields: ['title', 'name', 'content', 'summary', 'description'], },
  ...
  { type: 'messages', collection: 'chat_messages', permission: PERMISSIONS.COMMUNICATIONS_READ, titleFields: ['text', 'title'], textFields: ['text', 'title', 'body', 'content'], special: 'messages' },
];
...
function textOf(data: DocumentData, fields: string[]): string {
...
function titleOf(data: DocumentData, fields: string[]): string {
  for (const field of fields) if (typeof data[field] === 'string' && data[field].trim()) return data[field].trim().slice(0, 240);
  return 'Sem título';
}
...
function visible(data: DocumentData, identity: AuthenticatedIdentity, special?: Source['special']): boolean {
  ...
  const aclUsers = [data.ownerId, data.createdBy, data.assigneeId, data.assignedTo, data.userId].filter((v): v is string => typeof v === 'string');
  ...
  if (aclUsers.length && aclUsers.includes(identity.uid)) return true;
  return true;
}

function permissionFor(source: Source): Permission { return source.permission; }
...
function lexicalScore(query: string, title: string, text: string): number {
  ...
  const exact = hay.includes(q) ? 0.45 : 0;
  const tokenScore = tokens.reduce((sum, token) => sum + (hay.includes(token) ? 1 : 0), 0) / tokens.length;
  const titleBoost = tokens.reduce((sum, token) => sum + (title.toLowerCase().includes(token) ? 1 : 0), 0) / tokens.length * 0.35;
  return Math.min(1, exact + tokenScore * 0.2 + titleBoost);
}
...
async function embed(text: string): Promise<number[]> {
  ...
  const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-001:embedContent', {
    ...
    body: JSON.stringify({ model: 'models/gemini-embedding-001', content: { parts: [{ text: text.slice(0, 8000) }] }, taskType: 'RETRIEVAL_QUERY' }),
...
export async function enterpriseSearch(options: { q: string; scope?: SearchScope; type?: ContextType; workspaceId?: string; personId?: string; limit?: number; semantic?: boolean }): Promise<SearchResult[]> {
  ...
  const selected = SOURCES.filter((s) => requestedTypes.includes(s.type));
  const rows: Array<{ source: Source; id: string; data: DocumentData; title: string; text: string }> = [];
  for (const source of selected) {
    try {
      await import('@/server/authorization').then(({ requirePermission }) => requirePermission(permissionFor(source)));
      rows.push(...await sourceRows(source, identity));
    } catch { /* ACL failure means this source is omitted, never exposed. */ }
  }
  ...
  if (options.semantic !== false) {
    try { queryEmbedding = await embed(q); } catch { queryEmbedding = []; }
  }
  const candidateRows = filtered.slice(0, 600);
  const results: SearchResult[] = [];
  for (const row of candidateRows) {
    const lexical = lexicalScore(q, row.title, row.text);
    const stored = Array.isArray(row.data.embedding) ? row.data.embedding.filter((v): v is number => typeof v === 'number') : [];
    let semantic = stored.length && queryEmbedding.length ? cosine(queryEmbedding, stored) : 0;
    if (semantic < 0) semantic = 0;
    if (!stored.length && queryEmbedding.length && lexical > 0) {
      try {
        const vector = await embed(`${row.title}\n${row.text}`);
        semantic = cosine(queryEmbedding, vector);
        if (vector.length) await getAdminDb().collection('enterprise_search_index').doc(`${row.source.type}_${row.id}`.replace(/[^A-Za-z0-9_-]/g, '_')).set({ companyId: identity.companyId, sourceType: row.source.type, sourceId: row.id, title: row.title, text: row.text.slice(0, 12000), embedding: vector, updatedAt: Timestamp.now() }, { merge: true });
      } catch { semantic = 0; }
    }
    const recency = row.data.updatedAt instanceof Timestamp ? Math.max(0, 1 - (Date.now() - row.data.updatedAt.toMillis()) / (1000 * 60 * 60 * 24 * 365)) : 0;
    const score = Math.min(1, lexical * 0.55 + semantic * 0.4 + recency * 0.05);
    if (score > 0.03) results.push({ id: row.id, type: row.source.type, title: row.title, snippet: row.text.slice(0, 280), score, updatedAt: row.data.updatedAt instanceof Timestamp ? row.data.updatedAt.toDate().toISOString() : typeof row.data.updatedAt === 'string' ? row.data.updatedAt : undefined, workspaceId: typeof row.data.workspaceId === 'string' ? row.data.workspaceId : undefined, ownerId: typeof row.data.ownerId === 'string' ? row.data.ownerId : undefined, metadata: serialize({ sourceCollection: row.source.collection, status: row.data.status, entityType: row.data.entityType }) as Record<string, unknown> });
  }
  results.sort((a, b) => b.score - a.score);
  await writeAuditEvent({ companyId: identity.companyId, actorId: identity.uid, action: 'search.query', resourceType: 'enterprise_search', resourceId: identity.uid, metadata: { q: q.slice(0, 240), resultCount: results.length, semantic: Boolean(queryEmbedding.length) } });
...
export async function getKnowledgeGraph(entityType: ContextType, entityId: string, depth = 1): Promise<Record<string, unknown>> {
  const identity = await requireIdentity();
  const source = SOURCES.find((item) => item.type === entityType);
  if (!source) throw new Error('UNKNOWN_CONTEXT_TYPE');
  await import('@/server/authorization').then(({ requirePermission }) => requirePermission(source.permission));
  const db = getAdminDb();
  const ref = source.special === 'people' ? db.collection('companies').doc(identity.companyId).collection(source.collection).doc(entityId) : db.collection(source.collection).doc(entityId);
  const snap = await ref.get();
  if (!snap.exists || !visible(snap.data() ?? {}, identity, source.special)) throw new Error('NOT_FOUND');
  const node = { id: entityId, type: entityType, title: titleOf(snap.data() ?? {}, source.titleFields), data: serialize(snap.data() ?? {}) };
  const edges: Array<Record<string, unknown>> = [];
  const data = snap.data() ?? {};
  const relationMap: Array<[string, ContextType]> = [['projectId', 'projects'], ['workspaceId', 'people'], ['meetingId', 'meetings'], ['campaignId', 'campaigns'], ['taskId', 'tasks'], ['documentId', 'documents'], ['approvalId', 'approvals'], ['workflowId', 'workflows'], ['reportId', 'reports'], ['decisionId', 'decisions'], ['ownerId', 'people'], ['assigneeId', 'people']];
  for (const [field, targetType] of relationMap) {
    if (typeof data[field] !== 'string') continue;
    const target = SOURCES.find((item) => item.type === targetType); if (!target) continue;
    try { const targetSnap = (target.special === 'people' ? db.collection('companies').doc(identity.companyId).collection(target.collection) : db.collection(target.collection)).doc(data[field]).get(); const found = await targetSnap; if (found.exists && visible(found.data() ?? {}, identity, target.special)) edges.push({ field, to: { id: found.id, type: targetType, title: titleOf(found.data() ?? {}, target.titleFields) } }); } catch { /* inaccessible links disappear */ }
  }
  if (depth > 1 && edges.length) edges.push(...(await Promise.all(edges.slice(0, 12).map(async (edge) => getKnowledgeGraph((edge.to as { type: ContextType }).type, String((edge.to as { id: string }).id), depth - 1).catch(() => ({ nodes: [], edges: [] })) ))).flatMap((graph) => graph.edges as Array<Record<string, unknown>>));
  return { nodes: [node], edges };
}

export async function listKnowledgeObjects(limit = 100): Promise<SearchResult[]> { return enterpriseSearch({ q: '*', limit, semantic: false }); }
