...
export function validateWorkflowGraph(nodes:GraphNode[],edges:GraphEdge[]){
  ...
  for(const edge of edges){
    ...
    if(seenEdges.has(`${edge.source}>${edge.target}>${edgeKey(edge.label)}`))issues.push(`Edge duplicada: ${edge.id}`);
    seenEdges.add(`${edge.source}>${edge.target}>${edgeKey(edge.label)}`);
  }
...
export async function executeGraph<T extends GraphNode>(args:{
  nodes:T[];
  edges:GraphEdge[];
  isStartNode:(node:T)=>boolean;
  isJoinNode?:(node:T)=>boolean;
  joinExpected?:(node:T)=>number;
  route:(node:T,result:GraphResult,edges:GraphEdge[])=>GraphEdge[];
  execute:(node:T,context:{activations:GraphActivation[];results:Record<string,GraphResult>;runId:string})=>Promise<GraphResult>;
  runId:string;
}):Promise<Record<string,GraphResult>>{
  const byId=new Map(args.nodes.map(node=>[node.id,node]));
  const outgoing=new Map<string,GraphEdge[]>();
  for(const node of args.nodes)outgoing.set(node.id,[]);
  for(const edge of args.edges)if(byId.has(edge.source)&&byId.has(edge.target))outgoing.get(edge.source)!.push(edge);
  const activations=new Map<string,GraphActivation[]>();
  const results:Record<string,GraphResult>={};
  ...
  const startNode=async(node:T):Promise<void>=>{
    if(failure||started.has(node.id))return;
    const arrivals=activations.get(node.id)??[];
    const join=args.isJoinNode?.(node)===true;
    const expected=join?Math.max(1,args.joinExpected?.(node)??arrivals.length||1):1;
    if(arrivals.length<expected){
      if(!waiting.has(node.id)){
        const waiter=new Promise<void>(()=>{});
        waiting.set(node.id,waiter);
      }
      return;
    }
    started.add(node.id);
    try{
      const result=await args.execute(node,{activations:arrivals,results,runId:args.runId});
      results[node.id]=result;
      completed.add(node.id);
      const nextEdges=args.route(node,result,outgoing.get(node.id)??[]);
      for(const edge of nextEdges){
        if(!byId.has(edge.target))continue;
        const list=activations.get(edge.target)??[];
        list.push({from:node.id,edgeId:edge.id,result});
        activations.set(edge.target,list);
        if(!started.has(edge.target))void startNode(byId.get(edge.target)!);
      }
    }catch(error){
      failure=error;
      throw error;
  ...
  for(const node of args.nodes)if(args.isStartNode(node)){const list=activations.get(node.id)??[];list.push({from:null});activations.set(node.id,list);}
  for(const node of args.nodes)if((activations.get(node.id)?.length??0)>0){const task=startNode(node);tasks.add(task);task.finally(()=>tasks.delete(task));}
  while(tasks.size){await Promise.allSettled([...tasks]);}
  if(failure)throw failure;
  return results;
}

export function selectRoute(edges:GraphEdge[],route:string|string[]|undefined){
  if(route===undefined)return edges;
  const wanted=new Set((Array.isArray(route)?route:[route]).map(edgeKey));
  return edges.filter(edge=>wanted.has(edgeKey(edge.label))||(wanted.has('true')&&edgeKey(edge.label)==='')||(wanted.has('false')&&edgeKey(edge.label)==='false'));
}
