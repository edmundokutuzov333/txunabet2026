...
export const placeOrder = onCall({ secrets: [stripeApiKey], region: REGION, enforceAppCheck: true }, async (request) => {
  ...
  try {
    return { status: 'success', orderResult: await OrderService.getInstance(stripeApiKey.value()).processNewOrder({ ...parsed.data, customerId: identity.uid, companyId: identity.companyId }) };
  } catch (error) {
...
export const createPaymentIntent = onCall({ secrets: [stripeApiKey], region: REGION, enforceAppCheck: true }, async (request) => {
  ...
  try {
    const stripe = new Stripe(stripeApiKey.value(), { apiVersion: '2024-04-10', typescript: true });
    const paymentIntent = await stripe.paymentIntents.create({ amount: parsed.data.amount, currency: parsed.data.currency, payment_method: parsed.data.paymentMethodId, confirm: true, automatic_payment_methods: { enabled: true, allow_redirects: 'never' }, metadata: { firebaseUid: identity.uid, companyId: identity.companyId } });
    await db.collection('transactions').doc(paymentIntent.id).set({ id: paymentIntent.id, companyId: identity.companyId, userId: identity.uid, externalId: paymentIntent.id, source: 'stripe', amount: parsed.data.amount, currency: parsed.data.currency, status: paymentIntent.status === 'succeeded' ? 'succeeded' : 'pending', createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp(), metadata: {} });
