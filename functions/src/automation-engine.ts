...
async function activeAutomations(): Promise<admin.firestore.QueryDocumentSnapshot[]> {
  const snapshot = await db.collection('module_automations').where('active', '==', true).limit(MAX_AUTOMATIONS_PER_TICK).get();
  return snapshot.docs;
}

async function enqueueJob(input: { automationId: string; workflowId: string; companyId: string; trigger: string; payload: Record<string, unknown>; idempotencyKey: string }): Promise<string> {
...
async function scheduleCrons(date: Date): Promise<void> {
  ...
  for (const automation of snapshot.docs) {
    ...
    const cron = String((trigger as Record<string, unknown>).cron ?? '');
    const timezone = String((trigger as Record<string, unknown>).timezone ?? 'UTC');
    try {
...
async function executeStep(companyId: string, jobId: string, step: z.infer<typeof WorkflowStepSchema>, payload: Record<string, unknown>): Promise<void> {
  ...
  if (step.type === 'log') {
    logger.info('Automation workflow step', { companyId, jobId, stepId: step.id, message: String(config.message ?? step.name ?? '') });
    return;
  }
...
async function executeJob(job: Record<string, unknown>): Promise<void> {
  ...
  const workflowSnapshot = await db.collection('module_workflows').doc(workflowId).get();
  if (!workflowSnapshot.exists || workflowSnapshot.data()?.companyId !== companyId) throw new Error('WORKFLOW_NOT_FOUND');
  const workflow = WorkflowSchema.safeParse(workflowSnapshot.data());
  if (!workflow.success || workflow.data.active !== true) throw new Error('WORKFLOW_INVALID_OR_INACTIVE');
  const runRef = db.collection('automation_runs').doc(jobId);
  await runRef.set({ id: jobId, jobId, automationId: job.automationId, workflowId, companyId, status: 'running', attempts: job.attempts, startedAt: now(), updatedAt: now() }, { merge: true });
  for (const step of workflow.data.steps) {
    ...
    try {
      ...
      await stepRef.set({ status: 'succeeded', finishedAt: now(), updatedAt: now() }, { merge: true });
    } catch (error) {
...
async function processQueue(): Promise<void> {
  ...
  const candidates = [...pending.docs, ...running.docs];
...
export const triggerAutomationEvent = onCall({ region: REGION }, async (request) => {
  ...
  const jobs = await enqueueMatchingEvent(companyId, parsed.data.eventName, parsed.data.payload);
...
export const runAutomationNow = onCall({ region: REGION }, async (request) => {
  ...
  const parsed = AutomationSchema.extend({ payload: z.record(z.unknown()).default({}) }).safeParse(request.data);
  ...
  if (!workflowId) throw new HttpsError('failed-precondition', 'A automação não possui workflow.');
  const idempotencyKey = `manual_${parsed.data.id}_${Date.now()}_${request.auth.uid}`;
  const jobId = await enqueueJob({ automationId: parsed.data.id, workflowId, companyId, trigger: 'manual', payload: parsed.data.payload, idempotencyKey });
